...
namespace klang
{
    ...
    struct Version
    {
        ...
        bool
        atLeast(unsigned char M, unsigned char m, unsigned char b) const
        {
            return M < major || (M == major && m < minor) || (M == major && m == minor && b <= build);
        }

        bool
        below(unsigned char M, unsigned char m, unsigned char b) const
        {
            return !atLeast(M, m, b);
        }
...
#define DENORMALISE 1.175494e-38f
    ...
    struct constant
    {
        ...
        constexpr constant(double value) noexcept
            : d(value), f(static_cast<float>(value)), i(static_cast<int>(value)),
              inv(value == 0.0f ? 0.0f : static_cast<float>(1.0 / value))
        {
        }
        ...
        {
            return static_cast<float>(std::pow(d, x));
        }
...
#define CONSTANT constexpr constant
    ...
    /// @internal
    template <typename T>
    inline constexpr bool is_literal = std::is_trivially_constructible_v<T> && //
    ...
    constexpr bool
    are_scalars(Head &&head, Tail &&...tail)
    {
    ...
    inline constexpr power_t<BASE, EXP>
    power(BASE base, EXP exp)
    {
        ...
        {
            if constexpr (exp == (EXP)0)
            {
                return 1;
            }
            else if constexpr (exp == (EXP)1)
            {
                return base;
            }
            else if constexpr (exp == (EXP)2)
            {
                return base * base;
            }
            else if constexpr (exp == (EXP)3)
            {
                return base * base * base;
            }
            else if constexpr (exp == (EXP)4)
            {
                return base * base * base * base;
            }
            else if constexpr (exp == (EXP)-1)
    ...
    power(BASE base, EXP exp)
    {
        ...
        else if constexpr (std::is_floating_point_v<EXP>)
        {
            if (base == (BASE)10)
            {
                return (power_t<BASE, EXP>)::exp(
                    exp * (EXP)2.3025850929940456840179914546843642076011014886287729760333279009);
            }
            else if (exp == (EXP)0)
            {
    ...
    struct signal
    {
        ...
        signal &
        operator>>(signal &destination) const
        {
    ...
    inline static signal &
    operator>>(float input, signal &destination)
    { // NOTE: should this be signal, rather than float?
    ...
    struct signals
    {
        ...
        /// Stream operator (feedforward; allows further processing).
        signals &
        operator>>(signals &destination) const
        {
            destination = *this;
            return destination;
        }
        ...
        signals
        operator+(const signals x) const
        {
            signals s = *this;
            for (int v = 0; v < CHANNELS; v++)
            {
    ...
    /// Return a copy of the signal with each channel offset by x.
    template <int CHANNELS = 2>
    inline signals<CHANNELS>
    operator+(float x, const signals<CHANNELS> &y)
    {
        return y + x;
    }
    ...
    struct param : public signal
    {
        ...
        param &
        operator+=(const increment &increment)
        {
    ...
    struct params
    {
        ...
        params(param p) : parameters(new param[1]), size(1)
        {
            parameters[0] = p;
        }
    ...
    inline double
    fast_mod(double x, double y)
    {
        const unsigned long long i =
            (((unsigned long long)(x * (double(ULLONG_MAX) + 1.0) / y)) >> 12ULL) | 0x7FF0000000000000ULL;
    ...
    {
        const unsigned long long i =
            (((unsigned long long)(x * (double(ULLONG_MAX) + 1.0))) >> 12ULL) | 0x7FF0000000000000ULL;
    ...
    inline double
    fast_mod2pi(double x)
    {
        constexpr double         twoPi = (2.0 * 3.1415926535897932384626433832795);
        const unsigned long long i =
            (((unsigned long long)(x * (double(ULLONG_MAX) + 1.0) / twoPi)) >> 12ULL) | 0x7FF0000000000000ULL;
    ...
    fast_modi(double x)
    {
        ...
        const unsigned long long i =
            (((unsigned long long)(x * (double(ULLONG_MAX) + 1.0) * sizeInv)) >> 12ULL) | 0x7FF0000000000000ULL;
    ...
    struct Matrix
    {
        ...
        signals<4>
        operator<<(const signals<4> &in) const
        {
            return {v[0][0] * in.value[0] + v[0][1] * in.value[1] + v[0][2] * in.value[2] + v[0][3] * in.value[3],
                    v[1][0] * in.value[0] + v[1][1] * in.value[1] + v[1][2] * in.value[2] + v[1][3] * in.value[3],
                    v[2][0] * in.value[0] + v[2][1] * in.value[1] + v[2][2] * in.value[2] + v[2][3] * in.value[3],
                    v[3][0] * in.value[0] + v[3][1] * in.value[1] + v[3][2] * in.value[2] + v[3][3] * in.value[3]};
        }
    };

    inline signals<4>
    operator*(const signals<4> &in, const Matrix &m)
    {
        return {m[0][0] * in.value[0] + m[0][1] * in.value[1] + m[0][2] * in.value[2] + m[0][3] * in.value[3],
                m[1][0] * in.value[0] + m[1][1] * in.value[1] + m[1][2] * in.value[2] + m[1][3] * in.value[3],
                m[2][0] * in.value[0] + m[2][1] * in.value[1] + m[2][2] * in.value[2] + m[2][3] * in.value[3],
                m[3][0] * in.value[0] + m[3][1] * in.value[1] + m[3][2] * in.value[2] + m[3][3] * in.value[3]};
    }
    ...
    struct Phase : public param
    {
        ...
        Phase
        operator+(const increment &increment) const
        {
            if (increment.amount >= increment.size)
            {
                return value;
    ...
    struct Pitch : public param
    {
        ...
        text() const
        {
            ...
            const char *const        notes[12]  = {"C",     "C#/Db", "D",     "D#/Eb", "E",     "F",
    ...
    static struct SampleRate
    {
        ...
        SampleRate(float sr)
            : f(sr), i(int(sr + 0.001f)), d((double)sr), inv(1.f / sr), w(2.0f * pi * inv), nyquist(sr / 2.f)
        {
        }
    ...
    struct Control
    {
        ...
        float
        normalise(float value) const
        {
            return range() ? (value - min) / range() : std::clamp(value, 0.f, 1.f);
        }
        ...
        void
        setNormalised(float norm)
        {
            value.value = norm * range() + min;
        }
        ...
        template <typename TYPE>
        signal
        operator+(const Control &x) const
        {
            return value + (signal)x;
        }
        ...
        operator<<(TYPE &in)
        {
            value = in;
            return *this;
        } // assign to control with processing

        template <typename TYPE>
        Control &
        operator<<(const TYPE &in)
        {
            value = in;
            return *this;
        } // assign to control without/after processing
        ...
        {
            return value >> in;
        } // stream control to signal/object (allows processing)
        ...
        {
            return value >> in;
        } // stream control to signal/object (no processing)
    };
    ...
    template <int CHANNELS = 2>
    inline signals<CHANNELS>
    operator+(const Control &x, const signals<CHANNELS> &y)
    {
        return y + x.value;
    }
    ...
    {
        return {Caption::from(name), Control::Type::ROTARY, min, max, initial, size, NoOptions, initial, initial};
    }

    inline static Control
    Button(const char *name, Control::Size size = Automatic)
    {
        return {Caption::from(name), Control::Type::BUTTON, 0, 1, 0.f, size, NoOptions, 0.f, 0.f};
    }

    inline static Control
    Toggle(const char *name, bool initial = false, Control::Size size = Automatic)
    {
        return {Caption::from(name),                    //
                Control::Type::TOGGLE, 0,    1,         //
                initial ? 1.f : 0.f,   size, NoOptions, //
                initial ? 1.f : 0.f,                    //
                initial ? 1.f : 0.f};
    }
    ...
    {
        return {Caption::from(name), Control::Type::SLIDER, min, max, initial, size, NoOptions, initial, initial};
    }

    template <typename... Options>
    static Control
    Menu(const char *name, const Options... options)
    {
        ...
        return {Caption::from(name), Control::Type::MENU, 0, menu.size() - 1.f, 0, Automatic, menu, 0, 0};
    }

    template <typename... Options>
    static Control
    Menu(const char *name, Control::Size size, const Options... options)
    {
        Control::Options menu;
        const char      *strings[] = {options...};
        int              nbValues  = sizeof...(options);
        ...
        return {Caption::from(name), Control::Type::MENU, 0, menu.size() - 1.f, 0, size, menu, 0, 0};
    }

    inline static Control
    Meter(const char *name, float min = 0.f, float max = 1.f, float initial = 0.f, Control::Size size = Automatic)
    {
        return {Caption::from(name), Control::Type::METER, min, max, initial, size, NoOptions, initial, initial};
    }

    inline static Control
    PitchBend(Control::Size size = Automatic)
    {
        return {{"PITCH\nBEND"}, Control::Type::WHEEL, 0.f, 16384.f, 8192.f, size, NoOptions, 8192.f, 8192.f};
    }

    inline static Control
    ModWheel(Control::Size size = Automatic)
    {
        return {{"MOD\nWHEEL"}, Control::Type::WHEEL, 0.f, 127.f, 0.f, size, NoOptions, 0.f, 0.f};
    }
    ...
    struct Group
    {
        ...
        Group(const char *name, std::initializer_list<Control> controls)
            : name(name), size(Automatic), controls{controls}
        {
        }

        template <typename... Controls>
        Group(const char *name, Controls... ctrls)
            : name(name), size(Automatic), controls{std::forward<Controls>(ctrls)...}
        {
        }
        ...
        Group(const char *name, Control::Size size, std::initializer_list<Control> controls)
            : name(name), size(size), controls{controls}
        {
        }

        template <typename... Controls>
        Group(const char *name, Control::Size size, Controls... ctrls)
            : name(name), size(size), controls{std::forward<Controls>(ctrls)...}
        {
        }
    ...
    struct Controls : Array<Control, 128>
    {
        ...
        void
        operator+=(const Control &control)
        {
            items[count++] = control;
        }
        ...
        void
        operator=(std::initializer_list<klang::Group> controls)
        {
        ...
        void
        group(const char *name, unsigned int start, unsigned int length, Control::Size size = Automatic)
        {
    ...
    {
        void
        operator+=(const Preset &preset)
        {
            items[count++] = preset;
        }

        void
        operator=(const Presets &presets)
        {
        ...
        void
        operator=(std::initializer_list<Preset> presets)
        {
        ...
        void
        add(const char *name, const Values... values)
        {
            items[count].name = name;
    ...
    class buffer
    {
      ...
      public:
        ...
        buffer(float *buffer, int size) : owned(false), samples(buffer), size(size)
        {
            rewind();
        }

        buffer(float *buffer, int size, float initial) : owned(false), samples(buffer), size(size)
        {
            rewind();
            set(initial);
        }
        ...
        void
        attach(const buffer &buffer, int size = 0)
        {
        ...
        signal
        operator[](float offset)
        {
            return ((const buffer *)this)->operator[](offset);
        }

        signal
        operator[](float offset) const
        {
    ...
    namespace variable
    {
        class buffer
        {
          ...
          public:
            ...
            buffer(int size = 1, float initial = 0.f) : ptr(new klang::buffer(size, initial)), size(size)
            {
            }

            buffer(const klang::buffer &buffer) : ptr(new klang::buffer(buffer.size)), size(buffer.size)
            {
                *ptr = buffer;
            };
            ...
            operator klang::buffer &()
            {
                return *ptr;
            }

            operator const klang::buffer &() const
            {
                return *ptr;
            }
            ...
            operator signal &()
            {
                return ptr->operator signal &();
            }

            operator const signal &() const
            {
                return ptr->operator const signal &();
            }
            ...
            operator++(int)
            {
                return ptr->operator++(1);
            }

            variable::buffer &
            operator=(const klang::buffer &in)
            {
                resize(in.size);
                *ptr = in;
                return *this;
    ...
    namespace Generic
    {
        ...
        template <typename SIGNAL>
        inline SIGNAL
        operator+(float other, Output<SIGNAL> &output)
        {
        ...
        operator-(float other, Output<SIGNAL> &output)
        {
            return SIGNAL(other) - SIGNAL(output);
        }
        ...
        template <typename SIGNAL>
        inline SIGNAL
        operator+(Output<SIGNAL> &output, float other)
        {
            return SIGNAL(output) + other;
        }
        ...
        struct Function : public Generic::Modifier<SIGNAL>
        {
            ...
            template <typename T>
            inline void
            hash_combine(std::size_t &seed, const T &value) const
            {
                std::hash<T> hasher;
            ...
            std::size_t
            hash_tuple(const Tuple &tuple) const
            {
            ...
            std::tuple<Rest...>
            tail(const std::tuple<First, Rest...> &t) const
            {
            ...
            template <typename FunctionPtr>
            Function(FunctionPtr function) : function(std::forward<FunctionPtr>(function))
            {
            }

            template <typename FunctionPtr, typename... OtherArgs>
            Function(FunctionPtr function, OtherArgs... args) : function(std::forward<FunctionPtr>(function))
            {
                with(args...);
            }
            ...
            template <typename First, typename... Rest>
            const First &
            first(const First &first, Rest...) const
            {
                return first;
            }
            ...
            operator()(const FuncArgs &...args)
            {
                ...
                {
                    static_assert(sizeof...(FuncArgs) < (ARGS - 1),
                                  "Insufficient arguments: can only omit first argument");
                    // in = first(args...);
                    // std::get<0>(inputs) = in.value;
                    return *this;
                }
            ...
            template <typename... FuncArgs>
            float
            operator()(const FuncArgs &...args) const
            {
                signal              in     = this->in;
        ...
        template <typename TYPE, typename SIGNAL, typename... Args>
        SIGNAL
        operator+(TYPE &x, Function<SIGNAL, float> &func)
        {
            return func + (SIGNAL)x;
        }
        ...
        struct Graph
        {
            ...
            struct Point
            {
                ...
                valid() const
                {
                    return !std::isnan(x) && !std::isinf(x) && !std::isnan(y);
                } // NB: y can be +/- inf
            ...
            struct Series : public Array<Point, SIZE + 1>, Input<signal>
            {
                ...
                template <typename SIGNAL, typename... Args>
                void
                plot(const Generic::Function<SIGNAL, Args...> &f, const Axis &x_axis)
                {
                ...
                void
                plot(RETURN (*f)(ARG), const Axis &x_axis)
                {
                    constexpr int size = SIZE > 1024 ? 1024 : SIZE;
                ...
                bool
                operator==(const Series &in) const
                {
                    if (count != in.count)
                    {
                        return false;
            ...
            struct Axis
            {
                ...
                void
                from(const Series &series, double Point::*axis)
                {
                    if (!series.count)
                    {
                        return;
                    ...
                    for (unsigned int p = 0; p < series.count; p++)
                    {
                        ...
                        if (pt.valid() && !std::isinf(pt.*axis))
                        {
                            if (!points || pt.*axis < min)
                            {
                                min = pt.*axis;
                            }
                            if (!points || pt.*axis > max)
                            {
                                max = pt.*axis;
                            }
                            points++;
                    ...
                    if (std::abs(max) < 0.0000000001)
                    {
                        max = 0;
                    }
                    if (std::abs(min) < 0.0000000001)
                    {
                        min = 0;
                    }
                    if (std::abs(max) > 1000000000.0)
                    {
                        max = 0;
                    }
                    if (std::abs(min) > 1000000000.0)
                    {
                        min = 0;
                    }
                    if (min > max)
            ...
            Graph &
            operator()(double min, double max)
            {
                dirty      = true;
                axes.x.min = min;
                axes.x.max = max;
            ...
            Graph &
            operator()(double x_min, double x_max, double y_min, double y_max)
            {
                dirty      = true;
                axes.x.min = x_min;
                axes.x.max = x_max;
            ...
            template <typename SIGNAL, typename... Args>
            void
            plot(Generic::Function<SIGNAL, Args...> &function)
            {
            ...
            Graph &
            operator=(std::initializer_list<Point> values)
            {
                clear();
            ...
            template <typename... Args>
            Graph &
            operator<<(Function<Args...> &function)
            {
                plot(function.with());
                return *this;
            }
        ...
        template <int SIZE>
        inline typename Graph<SIZE>::Series &
        Graph<SIZE>::Point::operator>>(Graph<SIZE>::Series &series)
        {
            series.add(*this);
            return series;
        }
        ...
        struct Oscillator : public Generator<SIGNAL>
        {
          ...
          public:
            ...
            virtual void
            set(param frequency, param phase)
            {
                position = phase;
                set(frequency);
            }
            ...
            virtual void
            set(param frequency, relative phase)
            {
                set(frequency);
                set(phase);
            }
    ...
    using GeneratorOrModifier = typename std::conditional<
        std::is_base_of<Generator, TYPE>::value, Generic::Generator<SIGNAL>,
    ...
    struct GraphPtr
    {
        ...
        {
            check();
            return (Graph &)ptr->operator()(min, max);
        }
        ...
        operator[](int index) const
        {
            static const Graph::Series none = Graph::Series();
            return ptr ? ptr->operator[](index) : none;
        }
        ...
        {
            check();
            return (Graph &)ptr->operator()(x_min, x_max, y_min, y_max);
        }

        operator Graph::Series &()
        {
            check();
            return ptr->operator Graph::Series &();
        }
        ...
        Graph &
        operator+=(const Graph::Point pt)
        {
            check();
            return (Graph &)ptr->operator+=(pt);
        }
        ...
        Graph &
        operator=(std::initializer_list<Graph::Point> values)
        {
            check();
            return (Graph &)ptr->operator=(values);
        }
        Graph &
        operator+=(std::initializer_list<Graph::Point> values)
        {
        ...
        template <typename... Args>
        Graph &
        operator<<(Generic::Function<Args...> &function)
        {
            plot(function.with());
            return *this;
        }
        ...
        getAxes() const
        {
            static const Graph::Axes none;
            return ptr ? ptr->getAxes() : none;
        }

        void
        getAxes(Graph::Axes &axes)
        {
            check();
            ptr->getAxes(axes);
        }
    ...
    template <typename SIGNAL, typename... Args>
    inline GraphPtr &
    Generic::Function<SIGNAL, Args...>::operator>>(klang::GraphPtr &graph)
    {
        graph.plot(*this);
        return graph;
    }
    ...
    template <typename TYPE>
    static GraphPtr &
    operator>>(TYPE (*function)(TYPE), klang::GraphPtr &graph)
    {
        graph->plot(function);
        return graph;
    }
    ...
    template <typename SIGNAL, typename... Args>
    inline Graph &
    Generic::Function<SIGNAL, Args...>::operator>>(klang::Graph &graph)
    {
        graph.plot(*this);
        return graph;
    }
    ...
    template <typename TYPE>
    static Graph &
    operator>>(TYPE (*function)(TYPE), Graph &graph)
    {
        graph.plot(function);
        return graph;
    }
...
#define PROFILE(func, ...)                                                                                             \
    ...
    struct Debug : Input
    {
        ...
        THREAD_LOCAL static Buffer buf; // support multiple threads/instances
        ...
        template <typename Func, typename... Args>
        inline static double
        profile(int times, Func func, Args... args)
        {
            double sum = 0;
            while (times--)
            {
        ...
        struct Session
        {
            Session(float *, int size, Buffer::Content content)
            {
                ...
                if (buf.content != Buffer::Content::Notes)
                {
                    buf.clear(size);
                }
                buf.content = content;
        ...
        print(const char *format, ...)
        {
            if (console.length < 10000)
            {
                ...
                vsnprintf(string, 1024, format,
                          args);        // Safely format the string into the buffer
        ...
        getText(char *buf)
        {
            return console.getText(buf);
        }

        operator const signal &() const
        {
            return buf.operator const klang::signal &();
        }
    ...
    static Debug        debug;
    inline THREAD_LOCAL Debug::Buffer Debug::buf;
...
#define FUNCTION(type) (void (*)(type, Result<type> &))[](type x, Result<type> & y)
    ...
    struct Delay : public Modifier
    {
        ...
        tap(float delay) const
        {
            ...
            return buf[i] + fraction * (buf[j] - buf[i]);
        }

        signal
        lagrange(float delay) const
        {
            // Calculate the read position
            float read = static_cast<float>(position - 1) - delay;
            if (read < 0.f)
        ...
        tap() const
        {
            ...
            // Linear interpolation: buffer[i] + fraction * (buffer[j] - buffer[i])
            return buf[i] + last.fraction * (buf[j] - buf[i]);
        }
    ...
    struct Delay<0> : public Modifier
    {
        ...
        Delay() : buf(new klang::buffer(1, 0))
        {
            clear();
        }
    ...
    class Wavetable : public Oscillator
    {
      ...
      public:
        Wavetable(int size = 2048) : buf(size), size(size)
        {
        }

        template <typename TYPE>
        Wavetable(TYPE oscillator, int size = 2048) : buf(size), size(size)
        {
            operator=(oscillator);
        }
        ...
        virtual void
        set(param frequency, param phase) override
        {
        ...
        virtual void
        set(relative phase) override
        {
            offset = phase * float(size);
        }

        virtual void
        set(param frequency, relative phase) override
        {
            set(frequency);
            set(phase);
        }
    ...
    class Sample : public Oscillator
    {
      ...
      public:
        ...
        Sample &
        operator=(const klang::buffer &buffer)
        {
            size = buffer.size;
            Sample::buf.attach(buffer, size);
            return *this;
        }
    ...
    class Envelope : public Generator
    {
      ...
      public:
        ...
        Envelope &
        operator=(std::initializer_list<Point> points)
        {
            set(points);
            return *this;
        }
        ...
        void
        set(const std::vector<Point> &points)
        {
        ...
        void
        setLoop(int startPoint, int endPoint)
        {
            if (startPoint >= 0 && endPoint < static_cast<int>(points.size()))
            {
        ...
        {
            loop.reset();
            if (stage == Stage::Sustain && (point + 1) < static_cast<int>(points.size()))
            {
        ...
        void
        setTarget(const Point &point, float time = 0.0)
        {
        ...
        process()
        {
            ...
            case Stage::Sustain:
                ...
                { // envelop segment end reached
                    ...
                    else if ((point + 1) < static_cast<int>(points.size()))
                    {
                        ...
                        {                                                      // reached target point
                            ...
                            if ((point + 1) < static_cast<int>(points.size())) // new target point?
                            {
                                setTarget(points[point + 1], points[point].x);
                            }
                        }
      ...
      protected:
        ...
        void
        setTargetTime(const Point &point, float time = 0.0)
        {
            this->time = time;
        ...
        void
        setTargetRate(const Point &point, [[maybe_unused]] float rate = 0.0)
        {
            this->time = 0;
            if (point.x == 0)
            {
    ...
    struct ADSR : public Envelope
    {
      ...
      public:
        ...
        void
        set(param attack, param decay, param sustain, param release) override
        {
            A = attack;
            D = decay + 0.005f;
            S = sustain;
        ...
        void
        release(float time = 0.f, float level = 0.f) override
        {
            Envelope::release(time ? time : float(R), level);
        }

        bool
        operator==(Envelope::Stage stage) const
        {
    ...
    struct Operator : public OSCILLATOR, public Input
    {
        ...
        Operator &
        operator()(param f, relative phase)
        {
            OSCILLATOR::set(f, phase);
            return *this;
        }

        Operator &
        operator()(relative phase)
        {
        ...
        virtual Operator &
        operator=(const Envelope::Points &points)
        {
            env = points;
            return *this;
        }
        ...
        virtual Operator &
        operator=(const Envelope &points)
        {
            env = points;
            return *this;
        }
    ...
    /// Base class for UI / MIDI control
    struct Controller
    {
      protected:
        virtual event control([[maybe_unused]] int index, [[maybe_unused]] float value) {};
        virtual event preset([[maybe_unused]] int index) {};
        virtual event midi([[maybe_unused]] int status, [[maybe_unused]] int byte1, [[maybe_unused]] int byte2) {};

      public:
        virtual void
        onControl(int index, float value)
        {
            control(index, value);
        };
        ...
        virtual void
        onMIDI(int status, int byte1, int byte2)
        {
    ...
    class NoteBase : public Controller
    {
        ...
        class Controls
        {
          ...
          public:
            Controls &
            operator=(klang::Controls &controls)
            {
            ...
            operator[](int index)
            {
                return controls->operator[](index);
            }
      ...
      protected:
        virtual event
        on([[maybe_unused]] Pitch p, [[maybe_unused]] Velocity v)
        {
        }

        virtual event
        off([[maybe_unused]] Velocity v = 0)
        {
            stage = Stage::Off;
        }
        ...
        getSynth()
        {
            return static_cast<std::remove_pointer_t<decltype(synth)> *>(synth);
        }
      ...
      public:
        ...
        virtual bool
        stop([[maybe_unused]] Velocity v = 0)
        {
            stage = Stage::Off;
            return true;
        }
    ...
    struct Synth : public Effect
    {
        ...
        virtual event
        onControl(int index, float value) override
        {
            control(index, value);
        ...
        virtual event
        noteOn(int pitch, float velocity)
        {
        ...
        virtual event
        noteOff(int pitch, float velocity)
        {
            for (unsigned int n = 0; n < notes.count; n++)
            {
        ...
        virtual void
        process(buffer &buf) override
        {
            Effect::process(buf);
        }

        virtual void
        process(float *buf, int length, float *parameters = nullptr)
        {
            klang::buffer mono(buf, length);
    ...
    namespace Stereo
    {
        ...
        struct buffer
        {
            ...
            buffer(buffer const &buf) : left(buf.left), right(buf.right)
            {
                rewind();
            }
        ...
        namespace Mono
        {
            struct Note : public Stereo::Note, public klang::Mono::Generator
            {
                ...
                virtual bool
                process(Stereo::buffer buffer) override
                {
                    this->prepare();
                    while (!buffer.finished())
                    {
        ...
        struct Synth : public Effect
        {
            ...
            virtual void
            process(buffer buffer) override
            {
                Effect::process(buffer);
            }

            virtual void
            process(float **buffers, int length, float *parameters = nullptr)
            {
    ...
    template <typename SOURCE, typename DESTINATION>
    inline DESTINATION &
    operator>>(const SOURCE &source, DESTINATION &destination)
    {
        if constexpr (is_derived_from<Input, DESTINATION>())
        {
            destination.input(source); // input to destination (enables overriding of <<)
    ...
    namespace Generators
    {
        ...
        namespace Basic
        {
            ...
            struct Pulse : public Oscillator
            {
                ...
                using Oscillator::set;
                void
                set(param frequency, param phase, param duty)
                {
                    set(frequency, phase);
                    Pulse::duty = duty;
                }
            ...
            struct Noise : public Generator
            {
                ...
                process()
                {
                    out = rand() * 2.f / static_cast<float>(RAND_MAX) - 1.f;
                }
            };
        }; // namespace Basic
        ...
        namespace Fast
        {
            ...
            struct Increment
            {
                ...
                Increment
                operator+(const Increment &in) const
                {
            ...
            struct Phase
            {
                ...
                float
                operator+(const Phase &offset) const
                {

                    unsigned int phase = (position + offset.position) +
                                         0x40000000; // = quarter-turn (pi/2) phase offset range reduce to [0,pi]
            ...
            struct OSM
            {
                ...
                tick()
                {
                    ...
                    const State transition =
                        (State)(static_cast<int>(state) | (static_cast<int>(offset.position) < increment.amount
                                                               ? static_cast<int>(State::DownUpDown)
                ...
                saw()
                {
                    return saw(offset - col, tick());
                }

                inline float
                saw(const float p, const State state) const
                {
                    ...
                    case State::DownUpDown:
                        ...
                        break;
                    default:
                        return 0.f; // should never happen
                    }
                }
                ...
                pulse()
                {
                    return pulse(offset, tick());
                }

                inline float
                pulse(const float p, const State state) const
                {
                    // state machine action
                    switch (state)
                    {
    ...
    namespace Filters
    {
        ...
        namespace Biquad
        {
            ...
            struct Filter : Modifier
            {
                ...
                set(param f)
                {
                    this->set(f, root2.inv);
                }

                /// Set the filter cutoff and bandwidth
                void
                set(param f, relative bw)
                {
                    if (bw > 0)
                    {
    ...
    namespace Modifiers
    {
        ...
        struct Modal : public Modifier
        {
            ...
            void
            set(param f, param decay, param gain)
            {
                set(f, decay);
    ...
    struct Envelope::Follower : Modifier
    {
        ...
        struct AR : Modifier
        {
            ...
            void
            set(param attack, param release)
            {
    ...
    struct File
    {
        ...
        packed Chunk
        {
            packed ID
            {
                ...
                ID(char const *id) : id{id[0], id[1], id[2], id[3]}
                {
                }
        ...
        packed Header : Chunk
        {
            ...
            bool isWAV() const
            {
                return id == "RIFF" && format == "WAVE";
            }

            bool isAIFF() const
            {
                return id == "FORM" && format == "AIFF";
            }
        };
        ...
        packed WAV
        {
            ...
            packed Header : File::Header{Header() : File::Header("RIFF", "WAVE"){}} *header = nullptr;
            ...
            *data = nullptr;
            ...
            bool load()
            {
                ...
                // header
                header = (Header *)mem.ptr;
                if (!header->isWAV())
                {
                    return false;
                }
                mem.skip(sizeof(Header));

                // other chunks
                while (!mem.finished())
                {
                    const Chunk chunk = mem;
                    mem.rewind(sizeof(Chunk));
                    if (chunk.id == "fmt ")
                    {
                        format = (Format *)mem.ptr;
                    }
                    else if (chunk.id == "data")
                    {
                        if (mem.ptr + chunk.size > mem.end)
                        {
                            return false; // corrupt
                        }
                        data = (Data *)mem.ptr;
                    }
                    mem.skip(sizeof(Chunk) + chunk.size);
                }
                return true;
            }
